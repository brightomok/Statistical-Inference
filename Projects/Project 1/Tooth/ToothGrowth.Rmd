---
title: "Exploratory Graphing and Hypothesis Testing"
...
```{r pressure, fig.height=3, fig.width=5, echo=FALSE, out.extra='style="float:left"'}
theme <- theme(
...
ggplot(aes(x = factor(dose), y = len), data = ToothGrowth) + geom_boxplot(aes(fill = factor(dose))) + theme + scale_fill_brewer(palette="PuBu") + labs(title = "Figure 1")
```
...
```{r plot2, fig.height = 3, fig.width=5, echo = FALSE, out.extra='style="float:right"'}
ggplot(aes(x = supp, y = len), data = ToothGrowth) + geom_boxplot(aes(fill = supp)) + theme + scale_fill_brewer(palette="YlOrRd") + labs(title = "Figure 2")
```
...
```{r plot3, fig.height = 3, echo = FALSE}
